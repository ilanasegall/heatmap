...
/** KNOWN ISSUES:
    ...
    * historydropmarker shows as a click on the urlbar.
...
let maybeMenuClick = function(window,evt){
  ...
  Array.map(["toolbar-menubar","main-menubar"],function(id){
    ...
    OUT.menubar = id;
    OUT = override(OUT,aboutClick(evt,window))
    return OUT;
  })
}
...
let maybeTabBar = function(window,evt){
  ...
  if (evt.button == 0) {
    ...
    return override(OUT,aboutClick(evt,window));
  }
...
Track(browserOnly(function(window){
  window.addEventListener("keydown", function(evt) {
    ...
    let OUT = {ts:Date.now(), group: "key-nav"};
    OUT = override(OUT,tabAndWindowIds(window));

    // interesting ones... 13: enter, 38: up, 40: down
    OUT.action = evt.type;
    OUT.keycode = evt.keyCode
    OUT.target = evt.target.id;
    ['shiftKey','altKey','ctrlKey','metaKey'].forEach(function(att){
      if (evt[att]) OUT[att] = true
    })
    emit(OUT);
  },true);
}))
